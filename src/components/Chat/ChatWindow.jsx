    ...
    <section className="card chat-window" aria-labelledby="chat-heading">
      ...
      {!user && !authLoading ? (
        <div className="sign-in-cta">
          <p style={{ fontSize: '1.2rem', marginBottom: '16px' }}>Please sign in to access the assistant.</p>
          <button onClick={signInWithGoogle}>Sign In with Google</button>
        </div>
      ) : (
        <>
          <div className="messages-container" role="log" aria-live="polite">
            {messages.length === 0 && <p style={{ textAlign: 'center', color: 'var(--text-secondary)', marginTop: '40px' }}>Ask me anything about the upcoming election!</p>}
            {messages.map(msg => (
              <MessageBubble key={msg.id} message={msg} />
            ))}
            {isLoading && <div style={{ fontStyle: 'italic', color: 'var(--text-secondary)', marginLeft: '12px' }}>Assistant is typing...</div>}
